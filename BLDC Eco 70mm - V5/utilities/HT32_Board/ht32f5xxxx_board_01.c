...
{
  u32 bGpioId = LED_GPIO_ID[HT_LEDn];									//Asigna un puerto (ID)GPIO_PA=0  GPIO_PB=1 GPIO_PC=2 GPIO_PD=3 GPIO_PE=4 GPIO_PF=5
	
  u32 uGPIOPin = LED_GPIO_PIN[HT_LEDn];								//Asigna un GPIO_PIN_N (osea es un 1<<Numero_pin)
  AFIO_MODE_Enum uGPIOMode = LED_AFIO_MODE[HT_LEDn];	//Asigna la funcion del pin, en este caso es GPIO (AFIO_FUN_GPIO)
  HT_GPIO_TypeDef* GPIOx = GPIO_PORT[bGpioId];				//Aca hace la conversion de Numero de puerto a Direccion base de cada Puerto 

  /* Enable the GPIO_LED Clock                                                                              */
  /* Enable the AFIO Clock                                                                                  */
  _HT32F_DVB_ClockConfig(bGpioId);										//Le da clock al puerto 

  /* Configure the AFIO mode                                                                                */
  HT32F_DVB_GPxConfig(bGpioId, uGPIOPin, uGPIOMode);	//Config del puerto

  /* LED is off by default                                                                                  */
  HT32F_DVB_LEDOff(HT_LEDn);													//Valor por default 
